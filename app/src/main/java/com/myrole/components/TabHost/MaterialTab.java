package com.myrole.components.TabHost;

/**
 * Created by vikesh.kumar on 6/9/2016.
 */
...
public class MaterialTab implements View.OnTouchListener {
    ...
    public MaterialTab(Context ctx,boolean hasIcon) {
        ...
        if(Build.VERSION.SDK_INT < Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
            if(!hasIcon) {
                completeView = LayoutInflater.from(ctx).inflate(R.layout.layout_tab, null);

                text = (TextView) completeView.findViewById(R.id.text);
            }
            else {
                completeView = LayoutInflater.from(ctx).inflate(R.layout.layout_tab_icon, null);
            ...
            if (!hasIcon) {
                // if there is no icon
                completeView = LayoutInflater.from(ctx).inflate(R.layout.layout_material_tab, null);

                text = (TextView) completeView.findViewById(R.id.text);
            } else {
                // with icon
                completeView = LayoutInflater.from(ctx).inflate(R.layout.layout_material_tab_icon, null);
