...
import com.myrole.camera_ck.base.Constants;
...
public final class CameraHelper {
    ...
    public static void createApplicationFolder() {
        File f = new File(Environment.getExternalStorageDirectory(), File.separator + Constants.APPLICATION_DIR_NAME);
        f.mkdirs();
        f = new File(Environment.getExternalStorageDirectory(), File.separator + Constants.APPLICATION_DIR_NAME + Constants.IMAGE_DIR);
        f.mkdirs();
        f = new File(Environment.getExternalStorageDirectory(), File.separator + Constants.APPLICATION_DIR_NAME + Constants.VIDEO_DIR);
        f.mkdirs();
    }

    public static File generateStorageDir(@Nullable String pathToDirectory) {
        File mediaStorageDir = new File(Environment.getExternalStorageDirectory().getPath() + File.separator + Constants.APPLICATION_DIR_NAME, pathToDirectory);
    ...
    public static File getOutputMediaFile(@Configuration.MediaAction int mediaAction) {
        File mediaStorageDir = null;
        if (mediaAction == Configuration.MEDIA_ACTION_PHOTO) {
            mediaStorageDir = generateStorageDir(Constants.IMAGE_DIR);
        } else if (mediaAction == Configuration.MEDIA_ACTION_VIDEO) {
            mediaStorageDir = generateStorageDir(Constants.VIDEO_DIR);
        }
    ...
    public static File getOutputMediaFile(@Configuration.MediaAction int mediaAction, String ext) {
        File mediaStorageDir = null;
        if (mediaAction == Configuration.MEDIA_ACTION_PHOTO) {
            mediaStorageDir = generateStorageDir(Constants.IMAGE_DIR);
        } else if (mediaAction == Configuration.MEDIA_ACTION_VIDEO) {
        ...
        if (mediaStorageDir != null) {
            ...
            final String mediaStorageDirPath = mediaStorageDir.getPath();
            if (mediaAction == Configuration.MEDIA_ACTION_PHOTO) {
                mediaFile = new File(mediaStorageDirPath + File.separator + fileName + ext);
            } else if (mediaAction == Configuration.MEDIA_ACTION_VIDEO) {
                mediaFile = new File(mediaStorageDirPath + File.separator + fileName + ext);
            }
        }
    ...
    public static Size getMaxSizeRatio(List<Size> choices){
        if (choices.size() == 1) return choices.get(0);

        return Collections.max(choices, new CompareSizesByArea2());
    }

    public static Size getMaxSizeRatio(Size[] sizes){
        if (sizes == null || sizes.length == 0) return null;

        return getMaxSizeRatio(Arrays.asList(sizes));
    }
