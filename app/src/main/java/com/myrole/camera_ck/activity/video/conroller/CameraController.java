...
import com.myrole.camera_ck.activity.video.params.CameraParamsI;
...
public class CameraController implements CameraControllerI {
    ...
    public synchronized void openCamera(CameraParamsI params, int surfaceRotationDegrees) {
        ...
        if (targetSize.isAtLeastOneDimensionDefined()) {
            if (CameraUtil.isLandscapeAngle(surfaceRotationDegrees)) {
        ...
        for (CameraListener listener : mListeners) {
            listener.onCameraAdjustPreview(getPreviewSize());
    ...
    @Override
    public synchronized void setPreviewCallback(@Nullable PreviewCallback callback) {
        if (!isCameraOpen()) {
            return;
        }
    ...
    public synchronized void startPreview() {
        ...
        byte[] bufferByte = new byte[getPreviewSize().getWidthUnchecked() * getPreviewSize().getHeightUnchecked() * 3 / 2];
    ...
    public boolean supportsFlashMode(FlashMode flashMode) {
        return mParameters != null && mParameters.getSupportedFlashModes() != null && !Sets.intersection(
    ...
    public synchronized boolean setFlashMode(FlashMode flashMode) {
        boolean setFlashMode = false;
        if (isCameraOpen()) {
            Camera.Parameters parameters = mCamera.getParameters();
            setFlashMode = setFlashModeParams(flashMode, parameters);
            if (setFlashMode) {
